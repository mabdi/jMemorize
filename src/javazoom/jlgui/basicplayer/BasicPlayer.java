...
public class BasicPlayer implements BasicController, Runnable {
	...
	protected void initAudioInputStream() throws BasicPlayerException {
		try {
			...
			} else if (m_dataSource instanceof byte[]) {
				AudioInputStream strm = AudioSystem.getAudioInputStream(new ByteArrayInputStream((byte[]) m_dataSource));
				initAudioInputStream(  strm );
			}
			createLine();
...
//		inputStream.reset();
	...
	protected void notifyEvent(int code, int position, double value, Object description) {
		BasicPlayerEventLauncher trigger = new BasicPlayerEventLauncher(code, position, value, description,
				new ArrayList(m_listeners), this);
		trigger.start();
	}
	...
	public void open(byte[] audioInputStream) throws BasicPlayerException {
		if (audioInputStream != null) {
			m_dataSource = audioInputStream;
			initAudioInputStream();
		}
	}
}
