...
import jmemorize.tools.pearson.PearsonHelper;
...
public class CardPanel extends JPanel {
	...
	private class PearsonAction extends StyledTextAction {
		public PearsonAction() {
			super("prs");
		}

		public void actionPerformed(java.awt.event.ActionEvent e) {
			// TODO we suppose that m_cardSides has 2 elements and index 0 is
			// for front and index 1 for backside
			if (m_cardSides.size() != 2)
				return;

			CardSidePanel cardSidePanel1 = m_cardSides.get(0);// front side
			CardSidePanel cardSidePanel2 = m_cardSides.get(1);// front side
			if (cardSidePanel1.getText().getUnformatted().trim().isEmpty()
					|| cardSidePanel1.getText().getUnformatted().trim().split(" ").length == 0)
				return;
			String z = cardSidePanel1.getText().getUnformatted().trim().split(" ")[0];
			if (z.trim().isEmpty())
				return;
			...
			String s;
			try {
				s = new PearsonHelper().getWordData(text);
			} catch (SocketTimeoutException e3) {
				JOptionPane.showMessageDialog(null, "Time Out. Try Again.", "Error", JOptionPane.ERROR_MESSAGE);
				return;
			...
			if (s.length() > 0) {
				if (cardSidePanel2.getText().getUnformatted().isEmpty()) {
					cardSidePanel2.getTextPane().setText(s);
				} else {
					InputStream stream = new ByteArrayInputStream(s.getBytes(StandardCharsets.UTF_8));
					try {
						cardSidePanel2.addCardFile(new FileItem(stream, text + ".txt"));
					} catch (IOException e1) {
						e1.printStackTrace();
					}
				}
				cardSidePanel2.getTextPane().requestFocus();
			}

		}
	}

	private class DownloadPronAction extends StyledTextAction {
		public DownloadPronAction() {
			super("prn");
		}
		...
		public void actionPerformed(java.awt.event.ActionEvent e) {
			...
			if (z.trim().isEmpty())
				return;
			String text = z + ".mp3";
			String url = "https://ssl.gstatic.com/dictionary/static/sounds/de/0/" + text;
			URL u = null;
			try {
				u = new URL(url);
			...
			HttpURLConnection rsp;
			try {
				rsp = (HttpURLConnection) u.openConnection();
				rsp.setReadTimeout(15000);
				rsp.setConnectTimeout(15000);
				if (rsp.getResponseCode() != 200)
					return;

				cardSidePanel1.addCardFile(new FileItem(rsp.getInputStream(), text));
				cardSidePanel1.getTextPane().requestFocus();
			} catch (IOException e1) {
				e1.printStackTrace();
				return;
			}
		...
		public void actionPerformed(java.awt.event.ActionEvent e) {
			...
			if (editor != null && editor instanceof JTextPane) {
				for (CardSidePanel cardSidePanel : m_cardSides) {
					...
					try {
						cardSidePanel.addCardFile(new FileItem(file.toString(), file));
					} catch (IOException e1) {
	...
	private class SupAction extends AbstractStyledTextAction {
		public SupAction() {
			super("sup");
			addShortcut(KeyStroke.getKeyStroke(KeyEvent.VK_PLUS, CTRL_MASK | InputEvent.SHIFT_DOWN_MASK));
			addShortcut(KeyStroke.getKeyStroke(KeyEvent.VK_EQUALS, CTRL_MASK | InputEvent.SHIFT_DOWN_MASK));
		}
	...
	private class ShowCardSideButton extends JButton implements ActionListener {
		...
		public ShowCardSideButton(String text, int... sides) {
			...
			// Character(Integer.toString(index).charAt(0));
			// String actionName = "show-card-side-action-"+index;
			// getInputMap().put(KeyStroke.getKeyStroke(character,
			// InputEvent.CTRL_MASK), actionName);
		...
		private void updateText() {
			...
			// setFont(getFont().deriveFont(highlight ? Font.ITALIC :
			// Font.PLAIN));
			//// setBackground(highlight ? new Color(255, 180, 0) :
			// ColorConstants.CARD_SIDE_BAR_COLOR);
		}
	}
	...
	/**
	 * @param editable
	 *            <code>true</code> if front/back side textpanes should be
	 ...
	 * 
	 * @param observer
	 *            The text observer that is to be added as observer.
	 */
	...
	private JToolBar buildEditToolbar() {
		...
		toolBar.addSeparator();
		toolBar.add(createButton(new DownloadPronAction(), "spkr.png"));
		toolBar.add(createButton(new PearsonAction(), "pearson.png"));
		toolBar.setFloatable(false);
		return toolBar;
	}
