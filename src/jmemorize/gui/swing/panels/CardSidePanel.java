...
public class CardSidePanel extends JPanel {
	...
	private FilePanel m_imagePanel = new FilePanel();

	private List<FileItem> m_images = new LinkedList<FileItem>();
	...
	public void setCardFont(CardFont cardFont) {
		...
		m_textPane.setEditorKit(cardFont.isVerticallyCentered() ? new MyEditorKit() : new StyledEditorKit()); // HACK
		setText(fText);
	...
	private void setupTabBehavior() {
		...
		m_textPane.getInputMap(JComponent.WHEN_FOCUSED).put(ctrlTab, DefaultEditorKit.insertTabAction);
	}
	...
	private void updateImage() {
		...
		if (imgCount == 0) {
			...
			m_imagePanel.setItem(m_images.get(m_currentImage));//ImageToDisplay(m_images.get(m_currentImage).getImage());
