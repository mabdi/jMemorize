 ...
 * @author djemili
 */
public class CardTable extends JTable implements Settings.CardFontObserver, SelectionProvider, ProgramEndObserver {
	/**
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
			...
			if (c instanceof JLabel) {
				...
				if (modelColumn == m_tableModel.getSortingColumn()) {
					l.setIcon(
							m_tableModel.getSortingDir() == ViewModel.ASCENDING ? m_ascendingArrow : m_descendingArrow);
				} else {
					l.setIcon(null);
				}
			}
	...
	public class ViewModel extends AbstractTableModel {
		...
		private void resort() {
			if (m_cards != null) // CHECK
			{
				...
				Collections.sort(m_cards, m_orderDir == ASCENDING ? comparator : new ReverseOrder<Card>(comparator));
	...
	private static final String[] COLUMN_NAMES = { Localization.get(LC.FRONTSIDE), Localization.get(LC.FLIPSIDE),
			...
			Localization.get(LC.CREATED), Localization.get(LC.LAST_TEST), Localization.get(LC.EXPIRES),
			Localization.get(LC.PASSED), Localization.get(LC.MODIFIED), Localization.get(LC.RATIO) };
	...
	public String getToolTipText(MouseEvent event) {
		...
		case COLUMN_CREATED: // fall-through
		case COLUMN_MODIFIED:
			Date date = modelIndex == COLUMN_CREATED ? card.getDateCreated() : card.getDateModified();
			...
			if (column != -1) {
				...
				if (column == m_tableModel.getSortingColumn()) {
					...
					m_tableModel.setSorting(column,
							dir == ViewModel.ASCENDING ? ViewModel.DESCENDING : ViewModel.ASCENDING);
				}
