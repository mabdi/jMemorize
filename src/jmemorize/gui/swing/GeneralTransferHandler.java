...
public class GeneralTransferHandler extends TransferHandler {
	 ...
	 * for CUT-operations where we need to remove the formatted section from the
	 * original document.
	 */
	...
	public boolean canImport(JComponent comp, DataFlavor[] transferFlavors) {
		...
		if (comp instanceof JTextPane) {
			for (int i = 0; i < transferFlavors.length; i++) {
				if (transferFlavors[i] == FORMATTED_TEXT_FLAVOR || transferFlavors[i] == DataFlavor.stringFlavor) {
					return true;
				}
			}
		}
		...
		try {
			...
			if (comp instanceof CategoryTree) {
				...
				if (t.isDataFlavorSupported(FORMATTED_TEXT_FLAVOR)) {
					...
					FormattedTextSection fText = (FormattedTextSection) t.getTransferData(FORMATTED_TEXT_FLAVOR);
	...
	protected void exportDone(JComponent source, Transferable data, int action) {
		...
		try {
			if (data.isDataFlavorSupported(CARDS_FLAVOR)) {
				...
				FormattedTextSection formattedText = (FormattedTextSection) data.getTransferData(FORMATTED_TEXT_FLAVOR);
